...
import registerPlusExtension, { buildDelegationPrompt } from "../extensions/plus.ts";
...
test("builtin agents resolve from the bundled pi-subagents package", async () => {
	...
	assert.ok(names.includes("reviewer"), `expected reviewer in ${names.join(",")}`);
	assert.ok(agents.every((agent) => agent.source === "builtin"));
});

test("scanAgentsDir lists md stems and tolerates missing dirs", async () => {
	const dir = await mkdtemp(join(tmpdir(), "plus-agents-"));
	await writeFile(join(dir, "my-agent.md"), "---\nname: my-agent\n---\n");
	await writeFile(join(dir, "notes.txt"), "ignored");
	const agents = await scanAgentsDir(dir, "custom");
	assert.deepEqual(agents.map((agent) => agent.name).sort(), ["my-agent"]);
...
test("mergeAgentEntries: later groups override earlier by name", () => {
	const merged = mergeAgentEntries(
		[{ name: "x", source: "builtin" }, { name: "keep", source: "builtin" }],
...
test("buildDelegationPrompt: with task and without task", () => {
	assert.equal(buildDelegationPrompt("reviewer", "  review my diff  "), "Use reviewer to review my diff.");
	assert.match(
...
type CmdOptions = { description: string; handler: (args: string, ctx: unknown) => Promise<void> | void };
...
function makePi() {
	...
	return { commands, sent, pi: pi as unknown as Parameters<typeof registerPlusExtension>[0] };
}
...
test("extension registers doctor, listing, and per-agent commands", async () => {
	...
	assert.ok(names.includes("agent:scout"), `expected agent:scout in ${names.join(",")}`);
	const reviewer = commands.get("agent:reviewer");
	assert.ok(reviewer, "expected agent:reviewer command");
	assert.match(reviewer.description, /builtin/);
});
