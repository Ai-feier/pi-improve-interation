...
本仓库（pi-improve-interation）是小型 TypeScript 适配器库：无前端、无数据库、未配置 issue tracker。

- 适用：codebase-design、tdd、code-review、diagnosing-bugs、research、domain-modeling，对应适配器开发与维护主流程
- 按需：grill-with-docs（若引入 CONTEXT.md/ADR 体系）、prototype（若出现 UI 类需求）
- 暂不适用：to-spec、to-tickets、triage、wayfinder、setup-matt-pocock-skills，依赖 issue tracker 与标签配置；skill 全部保留不删，此处仅标注适用性
